...
- Added a `prefill` option to the extension actions `pi.sendUserMessage()` and `pi.sendUserMessageToContext()`, so an extension that injects a prompt can prime the response (or, with `{ thinking }`, the reasoning) it triggers. See [Extensions](docs/extensions.md#pisendusermessagecontent-options).
- Fixed a prefill being dropped when its message had to be queued: a prompt, steer, or follow-up that waits for the current turn now carries its prefill and primes the response it triggers once delivered, rather than being primed only when it happened to start a turn immediately.
